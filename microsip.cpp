...
// spoofsip.cpp : Defines the class behaviors for the application.
//
#include "stdafx.h"
#include "spoofsip.h"
#include "mainDlg.h"
...
BEGIN_MESSAGE_MAP(CspoofsipApp, CWinAppEx)
...
// CspoofsipApp construction

CspoofsipApp::CspoofsipApp()
{
...
// The one and only CspoofsipApp object

CspoofsipApp theApp;

// CspoofsipApp initialization
	...
	if (pj_ready && pjsua_var.state == PJSUA_STATE_RUNNING && tm.GetTime() - startTime.GetTime() > 10) {
		...
		if (blockDump) {
			...
			if (::MessageBox(NULL, message, caption, MB_YESNO | MB_ICONQUESTION) == IDYES) {
				MSIP::OpenURL(_T("https://www.spoofsip.org/downloads"));
			}
		}
		else {
#ifdef _GLOBAL_VIDEO
...
#else
			message.Format(_T("A crash happened. Make sure your system is healthy and you have enough free memory and hard disk space. You can try to uninstall SpoofSIP \"with configuration\", and install it again. If this is a softphone bug, you can contact us to help fix it. Tracking info: %s%s"), tm.Format(_T("%Y%m%d%H%M%S")), sent ? _T("Y") : _T("N"));
#endif
...
BOOL CspoofsipApp::InitInstance()
{
