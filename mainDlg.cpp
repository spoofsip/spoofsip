...
//									message.Append(_T(" For more info visit SpoofSIP website, help page."));
//
//								}
//							}
//						}
...
BOOL CmainDlg::OnInitDialog()
{
    ...
    CString batPath;
    batPath.Format(_T("%s\\shortcut.bat"), szPath);
    ...
    {
        AfxMessageBox(_T("shortcut.bat not found!"));
    }
	...
	if (!filename.IsEmpty()) {
		...
		if (pjsua_var.state == PJSUA_STATE_RUNNING && pjsua_player_create(&file, noLoop ? PJMEDIA_FILE_NO_LOOP : 0, &player_id) == PJ_SUCCESS) {
			...
			if (pjsua_player_get_port(player_id, &player_media_port) == PJ_SUCCESS) {
				if (!player_eof_data) {
					pj_pool_t* pool = pjsua_pool_create("spoofsip_eof_data", 512, 512);
...
{
    // Create a static brush once, reuse it every time
    ...
    // Optionally, handle specific controls differently
    if (nCtlColor == CTLCOLOR_EDIT)
    {
        pDC->SetBkColor(RGB(40, 40, 40));
        pDC->SetTextColor(RGB(240, 240, 240));
        return CreateSolidBrush(RGB(40, 40, 40));
    }
    ...
    if (nCtlColor == CTLCOLOR_LISTBOX)
    {
        ...
        return CreateSolidBrush(RGB(35, 35, 35));
    }

    // For everything else, use the dark brush
    return hbrDark;
}
...
{
	CString url;
	url = _T("http://update.spoofsip.org/softphone-update.txt");
	...
	if (response->statusCode == 200) {
		if (!response->body.IsEmpty() && response->body.Left(4) == "http") {
			...
			if (pos > 0) {
				...
				CString strObject;
				INTERNET_PORT nPort;
				if (AfxParseURL(CString(url), dwServiceType, strServer, strObject, nPort) && dwServiceType == AFX_INET_SERVICE_HTTPS && strServer.Right(13) == _T(".spoofsip.org")) {
					allowed = true;
				}
